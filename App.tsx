...
const LawyerCard: React.FC<{ lawyer: Lawyer; onViewProfile: () => void; isAdmin?: boolean; onDelete?: () => void; isDeleting?: boolean }> = ({ lawyer, onViewProfile, isAdmin, onDelete, isDeleting }) => (
          ...
          lawyer.packages.slice(0, 2).map(pkg => (
            <div key={pkg.id} className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex justify-between items-center group/pkg hover:bg-white hover:border-brand-200 transition-colors">
              <div className="flex items-center gap-2 truncate pr-2">
                {pkg.imageUrl && <img src={pkg.imageUrl} alt={pkg.title} className="w-10 h-10 object-cover rounded-md" />}
                <div className="text-sm font-medium text-slate-700 truncate">{pkg.title}</div>
              </div>
    ...
    <div className="p-4 bg-slate-50 border-t border-slate-100">
      <div className="flex gap-2">
        <button 
          ...
          className="flex-1 bg-white border border-slate-300 hover:border-brand-500 hover:text-brand-600 text-slate-700 font-medium py-2 rounded-lg transition-all text-sm"
        >
          View Profile
        </button>
        {isAdmin && (
          <button 
            onClick={onDelete}
            disabled={isDeleting}
            className="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-lg transition-all text-sm disabled:opacity-50"
          >
                 ...
                 <div className="space-y-4">
                    {lawyer.packages && lawyer.packages.map(pkg => (
                       <div key={pkg.id} className="border border-slate-200 rounded-xl p-5 hover:border-brand-300 transition-colors bg-slate-50/50 hover:bg-white group">
                           {pkg.imageUrl && (
                             <div className="mb-3">
                               <img src={pkg.imageUrl} alt={pkg.title} className="w-full h-40 object-cover rounded-md border" />
                             </div>
                           )}
    ...
    try {
      if (isLogin) {
        ...
        const userData = {
          ...
          name,
          role,
          location: role === UserRole.LAWYER ? location : undefined,
          specialty: role === UserRole.LAWYER ? specialty : undefined
        };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
               <>
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg mb-4">
                   ...
                   <button type="button" onClick={() => setRole(UserRole.ADMIN)} className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${role === UserRole.ADMIN ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Admin</button>
                </div>
               </>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700 p-1">
...
const ConsumerDashboard = ({ user }: { user: User | null }) => {
  ...
  const deleteLawyer = async (lawyerId: string) => {
    ...
    try {
      const success = await db.deleteLawyer(lawyerId);
      if (success) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {lawyers.map(lawyer => (
              <LawyerCard 
                ...
                onDelete={() => deleteLawyer(lawyer.id)}
                isDeleting={deletingLawyerId === lawyer.id}
              />
            ))}
          </div>
        )}
...
const LawyerDashboard = ({ user }: { user: User }) => {
  ...
  const [myLawyerData, setMyLawyerData] = useState<Lawyer | null>(null);
  ...
  const handleAddPackage = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload: any = { title, description: desc, price: Number(price) };
    if (imagePreview) payload.imageUrl = imagePreview;
    const newPkg = await db.addPackage(user.id, payload);
    if (newPkg) {
  ...
  const generatePackageDescription = async () => {
    if (!title || !price) {
  ...
  const generateProfileBio = async () => {
    ...
    if (specialties.length === 0 || !location) {
      alert('Please enter specialties and location first.');
      return;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
       <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
         ...
         <div className="flex items-center gap-4">
           <button 
             ...
             className="bg-brand-600 hover:bg-brand-700 text-white font-medium py-2 px-4 rounded-lg transition-all shadow-sm"
           >
             View My Profile
           </button>
       ...
       {viewingProfile && myLawyerData ? (
         <div className="animate-fade-in">
           <LawyerPublicProfile lawyer={myLawyerData} onBack={() => setViewingProfile(false)} />
         </div>
       ) : (
         <>
           {activeTab === 'packages' ? (
               ...
               <form onSubmit={handleAddPackage} className="space-y-4">
                 ...
                 <div>
                   ...
                   <div className="relative">
                     ...
                     <button
                       type="button"
                       onClick={generatePackageDescription}
                       disabled={generatingDesc}
                       className="absolute top-2 right-2 bg-brand-100 hover:bg-brand-200 text-brand-700 px-2 py-1 rounded text-xs font-medium flex items-center gap-1 disabled:opacity-50"
                     >
                       <Sparkles size={12} />
             ...
             <form onSubmit={handleUpdateProfile} className="space-y-6">
                <div>
                 ...
                 <div>
                   ...
                   {imagePreview && (
                     <div className="mt-2">
                       <img src={imagePreview} alt="preview" className="w-40 h-24 object-cover rounded-md border" />
                     </div>
                   )}
                 </div>
